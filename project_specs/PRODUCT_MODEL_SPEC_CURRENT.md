...
| Planned fixed-design workflow | A future public workflow should be framed as candidate/open decision: `--config <path>` for evaluation context plus a separate `--design <path>` YAML/JSON file, with linear evaluation only as the recommended first scope. | open decision | `didgeridoo_optimizer/pipeline/evaluate_linear.py`, `didgeridoo_optimizer/geometry/models.py`, `project_specs/USER_IO_CONTRACT_CURRENT.md` |
...
| Decide whether fixed-design evaluation is a supported user workflow. | Current lower-level APIs allow it, but no stable CLI or public design schema exists. A future workflow should avoid overclaiming linear results: an evaluated design is not A-E validation, material promotion, calibration approval, or physical proof. | open decision | `didgeridoo_optimizer/pipeline/evaluate_linear.py`, `didgeridoo_optimizer/geometry/models.py`, `project_specs/USER_IO_CONTRACT_CURRENT.md` |
