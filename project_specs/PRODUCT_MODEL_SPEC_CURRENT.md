...
| Decide whether best-design bundle files or best-design plots need separate config controls. | Current behavior always writes the best-design bundle when a best candidate exists, even when `reporting.save_plots` is false. | open decision | `didgeridoo_optimizer/pipeline/run_optimizer.py`, `didgeridoo_optimizer/reporting/export.py` |
